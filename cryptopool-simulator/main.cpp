...
// --- lightweight logging helpers for instrumentation ---
static inline bool cpp_trade_debug_enabled() {
    static int init = 0;
    static bool enabled = false;
    if (!init) {
        const char *env = getenv("CPP_TRADE_DEBUG");
...
static inline void cppdbg_emit(nlohmann::json const &payload) {
    if (!cpp_trade_debug_enabled()) return;
    std::string s = payload.dump();
    fprintf(stdout, "CPPDBG %s\n", s.c_str());
    fflush(stdout);
}

static inline bool cpp_step_trace_enabled() {
    static int init = 0;
    static bool enabled = false;
    if (!init) {
        const char *env = getenv("CPP_STEP_TRACE");
        enabled = (env != nullptr && env[0] != '\0' && env[0] != '0');
        init = 1;
    }
    return enabled;
...
static inline void cppstep_emit(nlohmann::json const &payload) {
    if (!cpp_step_trace_enabled()) return;
    std::string s = payload.dump();
    fprintf(stdout, "CPPSTEP %s\n", s.c_str());
    fflush(stdout);
}

static inline void log_step_iter_trace(const char *stage, const char *phase,
                                       std::pair<int,int> pair,
                                       int from, int to,
                                       money dx, money dy, money step, money price,
                                       money trial_volume, money previous_profit, money new_profit,
                                       money p_min, money p_max, money gas) {
    if (!cpp_step_trace_enabled()) return;
    nlohmann::json ev;
    ev["event"] = "STEP_ITER";
    ev["stage"] = stage;
    ...
    ev["step"] = step;
    ev["price"] = price;
    ev["trial_volume"] = trial_volume;
    ev["previous_profit"] = previous_profit;
    ev["new_profit"] = new_profit;
    ev["p_min"] = p_min;
    ev["p_max"] = p_max;
    ev["gas"] = gas;
    cppstep_emit(ev);
...
static inline void log_step_result_trace(const char *stage,
                                         std::pair<int,int> pair,
                                         int from, int to,
                                         money dx, money price, money profit,
                                         money gas, money p_min, money p_max) {
    if (!cpp_step_trace_enabled()) return;
    nlohmann::json ev;
    ev["event"] = "STEP_RESULT";
    ev["stage"] = stage;
    ev["pair"] = {pair.first, pair.second};
    ev["from_asset"] = from;
    ev["to_asset"] = to;
...
static inline nlohmann::json vec_to_json(std::vector<money> const &v) {
    nlohmann::json j = nlohmann::json::array();
    for (auto const &x : v) j.push_back(x);
    return j;
}

static inline void log_step_event(int config,
                                  int candle_index,
                                  u64 timestamp,
                                  std::pair<int,int> pair,
                                  const char *stage,
...
static inline void log_leg_event(int config,
                                 int candle_index,
                                 u64 timestamp,
                                 std::pair<int,int> pair,
                                 const char *stage,
                                 ...
                                 money ext_vol,
                                 int trades,
                                 std::vector<money> const &reserves_vec,
                                 std::vector<money> const &targets_vec,
                                 money xcp_profit,
                                 money xcp_profit_real,
                                 std::vector<money> const &price_oracle,
                                 std::vector<money> const &last_price,
                                 bool not_adjusted,
                                 bool is_light) {
    if (!cpp_trade_debug_enabled()) return;
    nlohmann::json ev;
    ...
    ev["stage"] = stage;
    ev["price_before"] = price_before;
    ev["price_after"] = price_after;
    ev["max_price"] = max_price;
    ev["min_price"] = min_price;
    ev["last_quote"] = last_quote;
    ev["dx"] = dx;
    ev["dy"] = dy;
    ev["volume_delta"] = volume_delta;
    ev["volume_total"] = volume_total;
    ev["ext_vol"] = ext_vol;
    ev["trades"] = trades;
    nlohmann::json reserves = nlohmann::json::array();
    for (auto const &x : reserves_vec) reserves.push_back(x);
    ev["reserves"] = reserves;
    nlohmann::json targets = nlohmann::json::array();
    for (auto const &p : targets_vec) targets.push_back(p);
    ev["target_prices"] = targets;
    ev["price_oracle"] = vec_to_json(price_oracle);
    ev["last_price"] = vec_to_json(last_price);
    ev["xcp_profit"] = xcp_profit;
    ev["xcp_profit_real"] = xcp_profit_real;
    ev["not_adjusted"] = not_adjusted;
    ev["is_light"] = is_light;
    cppdbg_emit(ev);
}

static inline void log_preleg_event(int config,
                                    int candle_index,
                                    u64 timestamp,
                                    std::pair<int,int> pair,
                                    const char *stage,
                                    ...
                                    money ext_vol,
                                    int pool_n,
                                    const std::vector<money> &reserves,
                                    const std::vector<money> &targets,
                                    const std::vector<money> &price_oracle,
                                    const std::vector<money> &last_price) {
    if (!cpp_trade_debug_enabled()) return;
    nlohmann::json ev;
    ev["event"] = "PRELEG";
    ev["config"] = config;
    ...
    for (auto const &x : reserves) r.push_back(x);
    ev["reserves"] = r;
    nlohmann::json t = nlohmann::json::array();
    for (auto const &p : targets) t.push_back(p);
    ev["target_prices"] = t;
    ev["price_oracle"] = vec_to_json(price_oracle);
    ev["last_price"] = vec_to_json(last_price);
    cppdbg_emit(ev);
...
static inline void log_tweak_event(int config,
    ...
    ev["xcp_profit"] = xcp_profit;
    ev["xcp_profit_real"] = xcp_profit_real;
    ev["allowed_extra_profit"] = allowed_extra_profit;
    ev["not_adjusted"] = not_adjusted;
    ev["is_light"] = is_light;
    ev["ma_half_time"] = ma_half_time;
    cppdbg_emit(ev);
}
...
vector<trade_data> get_data(std::string const &fname) {
    ...
    printf("parsing %s\n", name_to_open.c_str()); fflush(stdout);
    mapped_file mf;
...
bool get_all(json const &jin, int last_elems, vector<money> & price_vector, mapped_file &mf) {
    ...
    // Robustly extract datafile array to avoid operator[] asserts
    if (!jin.is_object()) {
        printf("get_all: config root is not an object\n"); fflush(stdout);
        return false;
    }
    auto it_df = jin.find("datafile");
    if (it_df == jin.end() || !it_df->is_array()) {
        printf("get_all: key 'datafile' missing or not array\n"); fflush(stdout);
        return false;
    }
    vector<string> dfiles;
    for (auto const &v : *it_df) {
        if (v.is_string()) dfiles.push_back(v.get<string>());
    }
    size_t N = dfiles.size();
    if (N == 1) N = 2;
    if (!(N == 2 || N == 3)) {
        printf("get_all: unsupported datafile count=%zu (expected 1,2,3)\n", dfiles.size()); fflush(stdout);
        return false;
    }
    vector<string> names;
    map<string, vector<trade_data>> all_trades;
    vector<pair<int, int>> pairs;
    printf("get_all: datafiles=%zu\n", dfiles.size()); fflush(stdout);
    if (N == 2) {
        pairs.push_back({0, 1});
        string name = dfiles[0];
        printf("get_all: using name '%s'\n", name.c_str()); fflush(stdout);
...
struct Trader {
    ...
    auto step_for_price_2(money p_min, money p_max, pair<int, int> p, money vol, money ext_vol) {
        ...
        const char *stage_str = (p_min > 0) ? "DIR2" : "DIR1";
        // + (step increases)
        while (true) {
            auto _dx_prev = _dx;
            auto _dy_prev = _dy;
            ...
            log_step_iter_trace(stage_str, "grow", p, _from, _to, _dx, _dy, step, price,
                                 v, previous_profit, new_profit, p_min, p_max, gas);
        ...
        while (true) {
            ...
            for (int ctr=0;ctr<2;ctr++) {
                ...
                log_step_iter_trace(stage_str, "shrink", p, _from, _to, _dx, _dy, step, price,
                                     v, previous_profit, new_profit, p_min, p_max, gas);
        ...
        if (previous_profit <= 0) _dx = 0;
        log_step_result_trace(stage_str, p, _from, _to, _dx, price, previous_profit, gas, p_min, p_max);
        return _dx;
    }
        ...
        auto b = d.pair1.second;
            ...
            if (!dir1_limit_valid) {
                ...
                log_step_event(simdata->num, (int)i, d.t, d.pair1, "DIR1",
                               p_before, 0, max_price, step,
                               vol, ext_vol, this->gas_fee, (int)N);
                if (step > 0) {
                    ...
                    ctr += 1;
                    // LEG event after DIR1 execution
                    log_leg_event(simdata->num, (int)i, d.t, d.pair1, "DIR1",
                                  p_before, last,
                                  max_price, min_price, last,
                                  step, dy,
                                  vol - prev_vol, vol,
                                  ext_vol, ctr,
                                  this->curve.x, this->curve.p,
                                  this->xcp_profit, this->xcp_profit_real,
                                  this->price_oracle, this->last_price,
                                  this->not_adjusted, this->is_light);
                }
            }
            ...
            if (!dir2_limit_valid) {
                ...
                if (step > 0) {
                    ...
                    last = N == 3 ? price_3(a, b) : price_2(a, b);
                    ctr += 1;
                    // LEG event after DIR2 execution
                    log_leg_event(simdata->num, (int)i, d.t, d.pair1, "DIR2",
                                  p_before, last,
                                  max_price, min_price, last,
                                  step, dy,
                                  vol - prev_vol, vol,
            ...
            money _midpoint = (_high + _low) / 2.L;
            money _norm = 0;
            if (N == 2) _norm = tweak_price_2(d.t, a, b, _midpoint);
            else        _norm = tweak_price_3(d.t, a, b, _midpoint);
            // TWEAK event
            log_tweak_event(simdata->num, (int)i, d.t, d.pair1,
                            (this->is_light ? "light" : "heavy"),
                            _norm, _midpoint, _high, _low,
                            this->curve.p, this->price_oracle, this->last_price,
                            this->xcp_profit, this->xcp_profit_real,
                            this->allowed_extra_profit,
                            this->not_adjusted, this->is_light,
                            this->ma_half_time);
            total_vol += vol;
            last_time = d.t;
...
int main(int argc, char **argv) {
    ...
    printf("Input config: %s\n", in_json_name.c_str()); fflush(stdout);
    ...
    printf("json_load ok\n"); fflush(stdout);
    if (!jin.is_object()) {
        printf("json_load: root is not an object\n"); fflush(stdout);
        return 0;
    }
    if (jin.count("configuration") == 0) {
        printf("json_load: key 'configuration' missing in %s\n", in_json_name.c_str()); fflush(stdout);
        return 0;
    }
    // Prefer iterator access to avoid operator[] assert on const objects
    auto it_conf = jin.find("configuration");
    if (it_conf == jin.end() || !it_conf->is_array()) {
        printf("json_load: 'configuration' is not an array in %s\n", in_json_name.c_str()); fflush(stdout);
        return 0;
    }
    int configurations = static_cast<int>(it_conf->size());
    if (configurations <= 0) {
    ...
    if (!jin.is_object()) {
        printf("json_load: root not object before get_all\n"); fflush(stdout);
        return 0;
    }
    if (jin.count("datafile") == 0) {
        printf("json_load: key 'datafile' missing before get_all (file=%s)\n", in_json_name.c_str()); fflush(stdout);
        return 0;
    }
